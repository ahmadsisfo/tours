<?php
class Madminsystemuser extends Model {
	public function addUser($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "user(username,user_group_id ,password, firstname, lastname, email, image, status,date_added) values('" . $this->db->escape($data['username']) . "', '" . (int)$data['user_group_id'] . "', '" . $this->db->escape(md5($data['password'])) . "', '" . $this->db->escape($data['firstname']) . "', '" . $this->db->escape($data['lastname']) . "', '" . $this->db->escape($data['email']) . "', '" . $this->db->escape($data['image']) . "', '" . (int)$data['status'] . "',  NOW())");
	}
