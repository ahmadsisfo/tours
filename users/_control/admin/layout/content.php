<?php
class CAdminLayoutContent extends Controller {
	private $error = array();

	public function index() {
		$this->load->bahasa('admin/layout/content');

		$this->document->setTitle($this->bahasa->get('heading_title'));

		$this->load->model('admin/layout/content');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->Madminlayoutcontent->addLayout($this->request->post);
			...
			$this->response->redirect($this->url->link('admin/layout/content', 'sign=' . $this->session->data['sign'] . $url, 'SSL'));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->Madminlayoutcontent->editLayout($this->request->get['layout_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $layout_id) {
				$this->Madminlayoutcontent->deleteLayout($layout_id);
			}
	...
	protected function getList() {
		$data = array_merge($this->bahasa->loadAll('default'),$this->bahasa->loadAll('admin/layout/type'));
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('admin/layout/content', 'sign=' . $this->session->data['sign'] . $url, 'SSL')
		);
		
		$data['insert'] = $this->url->link('admin/layout/content/add', 'sign=' . $this->session->data['sign'] . $url, 'SSL');
		$data['delete'] = $this->url->link('admin/layout/content/delete', 'sign=' . $this->session->data['sign'] . $url, 'SSL');
		...
		$layout_total = $this->Madminlayoutcontent->getTotalLayouts();

		$results = $this->Madminlayoutcontent->getLayouts($filter_data);

		foreach ($results as $result) {
			$data['layouts'][] = array(
				'layout_id' => $result['layout_type_id'],
				'name'      => $result['name'],
				'edit'      => $this->url->link('admin/layout/content/edit', 'sign=' . $this->session->data['sign'] . '&layout_id=' . $result['layout_type_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('admin/layout/content', 'sign=' . $this->session->data['sign'] . '&sort=name' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('admin/layout/content', 'sign=' . $this->session->data['sign'] . $url . '&page={page}', 'SSL');

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->bahasa->get('text_pagination'), ($layout_total) ? (($page - 1) * $this->config->get('limit_list')) + 1 : 0, ((($page - 1) * $this->config->get('limit_list')) > ($layout_total - $this->config->get('limit_list'))) ? $layout_total : ((($page - 1) * $this->config->get('limit_list')) + $this->config->get('limit_list')), $layout_total, ceil($layout_total / $this->config->get('limit_list')));
		...
		$this->response->setOutput($this->load->view('admin/layout/type', $data));
	}

	protected function getForm() {
		$data = array_merge($this->bahasa->loadAll('default'),$this->bahasa->loadAll('admin/layout/content'));
		...
		if (!isset($this->request->get['layout_id'])) {
			$data['action'] = $this->url->link('admin/layout/content/add', 'sign=' . $this->session->data['sign'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('admin/layout/content/edit', 'sign=' . $this->session->data['sign'] . '&layout_id=' . $this->request->get['layout_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('admin/layout/content', 'sign=' . $this->session->data['sign'] . $url, 'SSL');

		if (isset($this->request->get['layout_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$layout_info = $this->Madminlayoutcontent->getLayout($this->request->get['layout_id']);
		}
		...
		if (isset($this->request->post['layout_content'])) {
			...
			$data['layout_contents'] = $this->Madminlayoutcontent->getLayoutContents($this->request->get['layout_id']);
		...
		$this->response->setOutput($this->load->view('admin/layout/content', $data));
	}

	protected function validateForm() {
		$this->load->model('user');
		if (!$this->Muser->hasPermission('modify', 'admin/layout/content')) {
