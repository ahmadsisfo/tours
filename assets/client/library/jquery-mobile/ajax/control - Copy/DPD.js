...
$( document ).ready(function() {
	if(window.localStorage.getItem("rclogIN") == 1) {
		...
		alert("Selamat datang "+window.localStorage.getItem("rcuserNAME")+"\nAnda login sebagai admin "+window.localStorage.getItem("rcuserLEVEL")+" Kabupaten/Kota "+window.localStorage.getItem("rcuserNAMA_WIL"));
	}
});
...
var tpsPage = {
	...
	kelurahan : {
		...
		_read : function(id_kecamatan) {
			$('#tpsPage-kelurahan-table tbody').html('');
			var baruHtml = '';
			ajaxP.Request({
	...
	kabupaten : {
		...
		_read : function() {
			$('#tpsPage-kabupaten-table tbody').html('');
			ajaxP.Request({
				url: UChost+''+tpsPage.server+'c=kabupaten_read',
				success: function (result) {
					var baruHtml = ''; 
			...
			ajaxP.Request({
				...
				success: function (result) {
					if(result.status==false){
						...
						var kel_no=1;
						for (var i in result){
							if(result[i]['Kabupaten'].id == window.localStorage.getItem("rcuserID_WIL")){
...
var index 	= {
	...
	tab : {
		...
		_kecamatan : function(id_kabupaten){
			if(window.localStorage.getItem("rcuserID_WIL")==id_kabupaten){
				index.akses = true;
			}else{
				index.akses = false;
			}
						...
						for (var i in result){
							if(id_tps != result[i]['Tp'].id){
								...
								if(index.akses == true){
									...
									elementA += 'onclick="alert(\'Akses tidak diizinkan untuk Kabupaten/Kota ini.\');" ';
								}
...
var hitung 	= {
	...
	kelurahan : {
		...
		_read : function(id_kecamatan) {
			$('#hitung-kelurahan-table tbody').html('');
			var baruHtml = '';
			ajaxP.Request({
					...
					if(result.status==false){
						...
						for (var i in result){
							if(id_kecamatan != result[i]['Kecamatan'].id){
								if(i != 0){
									if(result[i-1][''].sah==null || result[i-1][''].sah==0){
										baruHtml += '<td colspan="3"><strong>Ket : data hitungan suara yang masuk masih NULL.</strong></td>';
									} else {
										baruHtml += '<td>'+result[i-1][''].sah+'</td><td>'+(golput-parseFloat(result[i-1][''].sah))+'</td>';
									}
									baruHtml += '<td>'+(parseFloat(result[i-1][''].sah)+parseFloat(result[i-1][''].tdk_sah))+'</td><td>'+result[i-1][''].digunakan+'</td></tr>';
					...
					if(result.status==false){
						...
						for (var i in result){
							if(id_kabupaten != result[i]['Kabupaten'].id){
								...
								if(window.localStorage.getItem("rcuserID_WIL")==result[i]['Kabupaten'].id){
