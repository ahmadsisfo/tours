...
$( document ).ready(function() {
	if(window.localStorage.getItem("rclogIN") == 1) {
		...
		alert("Selamat datang "+window.localStorage.getItem("rcuserNAME")+"\nAnda login sebagai "+window.localStorage.getItem("rcuserLEVEL")+" Kelurahan "+window.localStorage.getItem("rcuserNAMA_WIL"));
	}
});
...
var index 	= {
	...
	tab : {
		...
		_tps : function(id_kelurahan){
			if(window.localStorage.getItem("rcuserID_WIL")==id_kelurahan){
				index.akses = true;
			}else{
				index.akses = false;
			}
					...
					if(result.status==false){
						...
						for (var i in result){
							if(id_kelurahan != result[i]['Kelurahan'].id){
								...
								var warnaaside = "";
								if(window.localStorage.getItem("rcuserID_WIL")==result[i]['Kelurahan'].id){warnaaside = "background:#7FFFAA";}
								$('#index-kelurahan').append('<div class="ui-block-'+huruf+' ui-corner-all">'+
									...
									'<p class="aside" style="'+warnaaside+'">kel. '+result[i]['Kelurahan'].nama+'</p>'+
					...
					if(result.status==false){
						...
						for (var i in result){
							...
							if(id_kelurahan != result[i]['Kelurahan'].id){
								...
								if(window.localStorage.getItem("rcuserID_WIL")==result[i]['Kelurahan'].id){
