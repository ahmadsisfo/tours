...
if(window.localStorage.getItem("rclogIN") != 1) {
	...
	$(document).on('pageinit', '#statistik', function() {
		panelswip('statistik');
		statistik.tab._kabupaten();
		//login.form.checkaktivasi();
	});
		...
		_readUpdate : function(id_tps) {
			ajaxP.Request({
				...
				success: function (result) {
					...
					$('#index-tps-pt_l').html(parseFloat(result[0]['Tp'].pt_l));
					$('#index-tps-pt_p').html(parseFloat(result[0]['Tp'].pt_p));
					$('#index-tps-tdk_sah').html(result[0]['Tp'].tdk_sah);
					$('#index-tps-sah').html(result[0]['Tp'].sah);
					$('#index-tps-digunakan').html(result[0]['Tp'].digunakan);
...
var statistik 	= {
	...
	tps : {
		...
		_tab : function(id_kelurahan) {
			$('#statistik-tab-control').html(
				'<fieldset data-role="controlgroup" data-type="horizontal" data-mini="true">'+
					'<a href="#" onclick="statistik.tab._kabupaten();" class="ui-shadow ui-btn ui-icon-arrow-r ui-btn-icon-right ui-corner-all" >Sumatera Barat</a>'+
			...
			statistik.tps.kelurahan_select(id_kelurahan);
		},
		
		kelurahan_select : function(selected) {
			ajaxP.Request({
		...
		_read : function(id_kelurahan) {
			$('#statistik-tps').html('<br/>&nbsp Loading...<br/><img src="jquery-mobile/images/20.gif" />');
			var baruHtml = '';
			ajaxP.Request({
				...
				success: function (result) {
					if(result.status==false){
						...
						for (var i in result){
							if(huruf=="a"){
								huruf="b";
								s2.push(result[i]['Hitung'].suara);
							}else{
								huruf="a";
								s1.push(result[i]['Hitung'].suara);
								ticks.push('<span style="font-size:8pt">'+result[i]['Tp'].no+'</span>');
								
							}
							if(max_val<parseFloat(result[i]['Hitung'].suara)){
								max_val = parseFloat(result[i]['Hitung'].suara);
							}
							id_tps = result[i]['Tp'].id;
						}
						$('#statistik-tps').css("height",(ticks.length*40)+"px");
						max_val += 0.5*(Math.pow(10,((''+max_val).length-1)));
						//alert(JSON.stringify(s1+'---'+s2+'---'+ticks));
						var plot2 = $.jqplot('statistik-tps', [s1, s2], {
	...
	kelurahan : {
		...
		_tab : function(id_kecamatan) {
			$('#statistik-tab-control').html(
				...
				'</fieldset>');
			$('#statistik-tab-control').trigger('create');
			statistik.kelurahan.kecamatan_select(id_kecamatan);
		...
		_select : function() {
			ajaxP.Request({
				...
				success: function (result) {
					...
					$('#statistik-kelurahan-select').html(baruHtml);
					$('#statistik-kelurahan-select').val("").selectmenu('refresh');
				}
			});               
		},
		
		kecamatan_select : function(selected) {
			ajaxP.Request({
				...
				success: function (result) {
					...
					statistik.kelurahan._select();
				}
			});
		},
		
		_read : function(id_kecamatan) {
			$('#statistik-kelurahan').html('<br/>&nbsp Loading...<br/><img src="jquery-mobile/images/20.gif" />');			var baruHtml = '';
			ajaxP.Request({
				...
				success: function (result) {
					if(result.status==false){
						...
						for (var i in result){
							if(huruf=="a"){
								huruf="b";
								s2.push(result[i][''].suara);
							}else{
								huruf="a";
								s1.push(result[i][''].suara);
								ticks.push('<span style="font-size:8pt">'+result[i]['Kelurahan'].nama.replace(" ",'').replace(" ",'').replace(" ",'')+'</span>');
								
							}
							if(max_val<parseFloat(result[i][''].suara)){
								max_val = parseFloat(result[i][''].suara);
							}
							id_kelurahan = result[i]['Kelurahan'].id;
						}
						$('#statistik-kelurahan').css("height",(ticks.length*50)+"px");
						max_val += (Math.pow(10,((''+max_val).length-1)));
						
						//alert(JSON.stringify(s1+'---'+s2+'---'+ticks));
						var plot2 = $.jqplot('statistik-kelurahan', [s1, s2], {
							animate: !$.jqplot.use_excanvas,
	...
	kecamatan : {
		...
		_tab : function(id_kabupaten) {
			$('#statistik-tab-control').html(
				'<fieldset data-role="controlgroup" data-type="horizontal" data-mini="true">'+
					'<a href="#" onclick="statistik.tab._kabupaten();" class="ui-shadow ui-icon-arrow-r ui-btn-icon-right ui-btn ui-corner-all" >Sumatera Barat</a>'+
				...
				'</fieldset>');
			$('#statistik-tab-control').trigger('create');
			statistik.kecamatan.kabupaten_select(id_kabupaten);
		...
		_select : function(selected) {
			ajaxP.Request({
				...
				success: function (result) {
					...
					$('#statistik-kecamatan-select').html(baruHtml);
					$('#statistik-kecamatan-select').val("").selectmenu('refresh');
				}
			});
		},
		
		kabupaten_select : function(selected) {
			ajaxP.Request({
				...
				success: function (result) {
					...
					statistik.kecamatan._select();
				}
			});      
		},
		
		_read : function(id_kabupaten) {
			$('#statistik-kecamatan').html('<br/>&nbsp Loading...<br/><img src="jquery-mobile/images/20.gif" />');
			var baruHtml = '';
			ajaxP.Request({
					...
					if(result.status==false){
						...
						for (var i in result){
							if(huruf=="a"){
								...
								huruf="a";
								s1.push(result[i][''].suara);
								ticks.push('<span style="font-size:8pt">'+result[i]['Kecamatan'].nama.replace(" ",'').replace(" ",'')+'</span>');
								
							}
							if(max_val<parseFloat(result[i][''].suara)){
								max_val = parseFloat(result[i][''].suara);
							}
							id_kabupaten = result[i]['Kecamatan'].id;
						}
						$('#statistik-kecamatan').css("height",(ticks.length*60)+"px");
						max_val += (Math.pow(10,((''+max_val).length-1)));
						
						//alert(JSON.stringify(s1+'---'+s2+'---'+ticks));
						var plot2 = $.jqplot('statistik-kecamatan', [s1, s2], {
							animate: !$.jqplot.use_excanvas,
	...
	kabupaten : {
		...
		_tab : function() {
			$('#statistik-tab-control').html(
				'<fieldset data-role="controlgroup" data-type="horizontal" data-mini="true">'+
					'<a href="#" onclick="statistik.tab._kabupaten();" class="ui-shadow ui-btn ui-corner-all" >Sumatera Barat</a>'+
					'<select id="statistik-kabupaten-select" onchange="statistik.tab._kecamatan(this.value)">'+
					'</select>'+
				'</fieldset>');
			$('#statistik-tab-control').trigger('create');
			statistik.kabupaten._select();
		},
		...
		_read : function() {
			$('#statistik-kabupaten').html('<br/>&nbsp Loading...<br/><img src="jquery-mobile/images/20.gif" />');
			ajaxP.Request({
					...
					if(result.status==false){
						...
						for (var i in result){
							if(huruf=="a"){
								...
								huruf="a";
								s1.push(result[i][''].suara);
								ticks.push('<span style="font-size:8pt">'+result[i]['Kabupaten'].nama.replace(" ",'').replace(" ",'')+'</span>');
								
							}
							if(max_val < parseFloat(result[i][''].suara)){
								//alert(result[i][''].suara);
								max_val = parseFloat(result[i][''].suara);
							}
							
							id_kabupaten = result[i]['Kabupaten'].id;
						}
						$('#statistik-kabupaten').css("height",(ticks.length*100)+"px");
						max_val += 0.5*(Math.pow(10,((''+max_val).length-1)));
						//alert(JSON.stringify(max_val));
						var plot2 = $.jqplot('statistik-kabupaten', [s1, s2], {
							animate: !$.jqplot.use_excanvas,
